...
/** Shared UTF-8 cap for wrapper diagnostics and relayed error detail. */
export const MAX_LOG_BYTES = 16_384;

/** Clips text to MAX_LOG_BYTES of UTF-8, flagging truncation. */
export function clipText(text: string): { text: string; truncated: boolean } {
  if (Buffer.byteLength(text, "utf8") <= MAX_LOG_BYTES) {
    return { text, truncated: false };
  }
  return {
    text: Buffer.from(text, "utf8").subarray(0, MAX_LOG_BYTES).toString("utf8"),
    truncated: true,
  };
}
...
/** Writes one wrapper-owned diagnostic through the error-detail redaction
 * boundary. It deliberately does not restore a newline lost to the cap: doing
 * so would make stderr's byte contract differ from boundErrorDetail's. */
export function writeRedactedStderr(
  text: string,
  writer: (chunk: string) => unknown = (chunk) => process.stderr.write(chunk),
): string {
  const safeText = boundErrorDetail(text);
  writer(safeText);
  return safeText;
}
