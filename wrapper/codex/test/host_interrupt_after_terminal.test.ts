// An interrupt that lands after the SDK terminal was observed — while the
// host is still finishing the turn (here: the permission observation's
// retry window) — must not mark a finished turn as interrupted, or an
// approved reset would be dropped for a turn the operator never cut short
// (issue #347 review, sibling of R2).
...
describe("CodexHost — interrupt after the terminal was observed", () => {
  it("reports the terminal without `interrupted` when the interrupt lands in the post-terminal window", async () => {
    ...
    expect((ends[0] as { interrupted?: unknown }).interrupted).toBeUndefined();
  });
});
