...
// runCodexCli() invocation and be removed in `finally`, mirroring issue
// #379's lesson (a listener left behind after `run()` settles accumulates
// across repeated invocations in the same process). A fake host (not a
// real exec/app-server backend) is enough here: this pin is about listener
// bookkeeping, not subprocess escalation, which the real-process pin
// covers separately.
...
async function runOnceAndSigterm(): Promise<{ closed: boolean }> {
  ...
  await started;
  // Same in-process technique as the real-process pin: fires the listener
  // `runCodexCli` itself registered, standing in for a real OS signal.
  process.emit("SIGTERM" as never);
  await running;
  return { closed };
}

describe("Codex CLI SIGTERM handler lifecycle (issue #391)", () => {
