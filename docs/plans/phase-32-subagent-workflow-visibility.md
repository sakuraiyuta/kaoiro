...
`AgentCard` に「頭上リング」(CSS-only の光点周回アニメーション)として
...
| 32-3 | dashboard: `task` envelope 受信 + `AgentCard` 頭上リング | あお | ✅ | `protocol.ts` に `TaskPayload`/`taskOf`/`parseTasks`/`applyTaskEnvelope`。`App.svelte` は `tasks` を `agents` とは別の accumulator として保持(ADR-0019 F2: 親の state_change slot を上書きしない)。`AgentCard.svelte` は `.sprite`/`.face` を包む `.sprite-slot`(position: relative)+ `.task-ring`(`transform: rotate` で ::before の光点を周回、画像アセット無し)。`prefers-reduced-motion` は `app.css` の既存グローバル規則(`animation-duration: 0.01ms !important` 等)でカバーされ、per-component override 不要。activeTaskCount は on/off のみに使い、数値表示はしない(こはくスコープ判断) |
...
- Codex エンジンでの同等対応: 実測(`@openai/codex-sdk@0.144.1` の型
  ...
  のみ行い、実装はしない。
- 活性タスク数の数値表示、`AgentDetail` への追加表示: 将来の別提案とし
  今回は対象外。
