...
- [ ] `wrapper/codex` が `EngineAdapter` interface を実装し、`thread.runStreamed()` / `codex.resumeThread(id)` を通しで駆動できる (毎ターン `codex exec` spawn の process モデル)。
- [ ] [ADR-0033](../adr/0033-permission-model-dual-axis.md) F1 の agent-level `ext.permission = {sandbox, approval}` が envelope に載る。Claude 6 mode → 二軸 mapping table (F2) が `wrapper/claude-code` に実装され、`ext.permission_mode` は 1 リリース窓並置で後方互換。
- [ ] dashboard の権限表示 (AgentCard / AgentDetail) が `ext.permission` 由来の二軸バッジになり、操作 UI は engine-native (Claude = mode セレクト、Codex = sandbox セレクト + network toggle、二軸換算ラベル併記。ADR-0033 F4)。
- [ ] 共通 Tool 記述層に inter-agent tools (`mcp__kaoiro__send_to_agent` / `list_agents` / `whoami`) が移送され、Claude (in-process MCP) / Codex (同梱 MCP bridge、ADR-0032 F5) 両方から呼び出せる。
- [ ] Codex 側の AskUserQuestion 相当 (`ask_user_question`、MCP bridge 経由) が動作し、operator へ質問が届き `waiting_question` が成立する。Claude native 側の挙動に影響なし。
...
- [ ] LaunchDialog の engine セレクトが有効化 (host の `capabilities` が 2 種以上のときのみ表示)。engine 選択 → model リスト (Codex は curated 静的カタログ: gpt-5.6-sol/terra/luna, gpt-5.5, gpt-5.4-mini) → optional effort の三段選択が動作。
- [ ] capabilities フィールドの値が `claude-code` / `codex` にリネーム完了。旧値 `claude` は 1 リリース窓のサイレント正規化 + deprecation warn (ADR-0032 F4a)。
- [ ] engine 別 session 列挙が動作: Claude adapter は既存 JSONL 列挙、Codex adapter は `~/.codex/sessions/**/rollout-*.jsonl` の `session_meta.cwd` 照合 (ADR-0032 F8)。
- [ ] fuji / kuroe / ao / momo の 4 persona 全てが Codex adapter 上でも動作し、口調・態度が Claude 版と同等に再現される ([Q1](../open-questions/codex-personality-injection-efficacy.md) 解決。注入は `developer_instructions`、built-in `personality` config の干渉も検証)。
...
- [Q1 codex-personality-injection-efficacy](../open-questions/codex-personality-injection-efficacy.md) — phase-14 完了判定に含む (代表 persona × Codex 動作確認。注入経路は `developer_instructions` で確定済み、実効性評価のみ残)

旧 Q2 (envelope schema) / Q3 (UI 語彙) / Q5 (model カタログ) / Q6 (互換窓) は 2026-07-10 の実 SDK 検証 + spec-elicitation で解決し close 済み (決定は [ADR-0032](../adr/0032-codex-adapter.md) / [ADR-0033](../adr/0033-permission-model-dual-axis.md) に追補)。
