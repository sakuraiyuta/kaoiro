...
- [ ] `request_compact` に optional `resume_prompt` パラメータを追加
      (Claude wrapper のみ。承認ダイアログには reason 同様 echo)
- [ ] `compact_boundary` 観測時、固定前置テンプレート + resume_prompt
      逐語を直列化 instruction queue へ user turn として注入
- [ ] 省略時の挙動が現状と完全一致することの回帰テスト
- [ ] 注入経路 MUST (固定テンプレート、model 由来逐語は resume_prompt
      本文のみ) の反映とテスト

受け入れ: resume_prompt 付き compact 承認 → 圧縮完了後に agent が
自動で作業再開する実機確認。server 変更ゼロ。
