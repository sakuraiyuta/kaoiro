...
Related security topics: [Security boundaries](../../architecture/security-boundaries.md), [Security threat model](../../architecture/security-threat-model.md), [Authentication and authorization](authentication-authorization.md), [Tool authorization](tool-authorization.md), [Security release audit](../../operations/security-release-audit.md).
...
- **Reserved-token vocabulary (where the copies live)**: the dashboard's check
  ...
  The server keeps its own `@reserved_session_commands`
  (`agents_channel.ex`) as the independent enforcement boundary above. A drift
  between the two shows up as a reserved token reaching the engine, or as a
  dashboard guard that never fires — change them together.
...
[auth-and-authz](../../architecture/security-boundaries.md).
