...
| Client authentication | `KAOIRO_CLIENT_TOKENS` | Effectively required | Multiple `token:role` values. role = `admin` / `operator` / `viewer`. Missing value rejects all clients (fail-closed). |
...
| OAuth identity authentication | `KAOIRO_OAUTH_*` / `KAOIRO_OAUTH_ALLOWLIST_PATH` | Optional | Google / GitHub / Nextcloud. Details: [deployment guide 1.6](../../operations/network-and-login.md#16-configure-oauth-login-optional-adr-0042--issue-65). |
...
- Fields the wizard does not ask (`codex.backend` / `codex.chatgpt_plan` /
  `codex.extra_models` / `codex.internal_subagents` /
  `antigravity.extra_models` / `antigravity.cli_path` /
  `antigravity.probe_timeout_ms` / `antigravity.max_sandbox` /
  ...
  [Runner configuration](runner.md)'s "Codex 設定" /
...
- Related specs: [protocol](../protocol/envelope.md), [threat-model](../../architecture/security-threat-model.md)
...
- [Multi-host deployment architecture](../../architecture/deployment.md#setup-wizards).
