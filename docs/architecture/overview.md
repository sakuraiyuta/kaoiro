...
- Obtain one agent's state from the message sequence of an engine SDK and
  represent it as a state machine ([architecture](system-overview.md)). Claude
...
See [non-goals](scope.md) for work outside the scope.
...
- SHOULD: The wrapper uses TypeScript + an engine SDK, the server uses
  ...
  [architecture](system-overview.md) for details.
...
- Related specs: [architecture](system-overview.md), [non-goals](scope.md),
