...
長さの上限そのものをどう担保するかも本件で決める:

| 案 | 内容 | メリット | デメリット |
|----|------|----------|-----------|
| L1 | 上限なし (運用者責任) | 実装ゼロ。`user-footer.md` の自由度を損なわない | 肥大に気付く契機がない |
| L2 | 閾値超えで server ログに warn | 肥大が可視化される | 閾値の根拠が要る。warn が常時出ると無視される |
...
- [ADR-0045](../adr/0045-footer-file-externalization.md) が入れば文面の
  差し替えに server 再ビルドが不要になり、「案 A から始めて不足分を
  計測する」進め方の試行コストが下がる
