---
title: 協調指針の具体文面と長さ
description: 共通フッターへ追記する協調行動指針の文面 (案 A/B) を決める。長さ担保の機構は ADR-0045 F5 (ログ常時可視化) で決着済み。
status: open
urgency: medium
blocks: []
opened: 2026-07-28
...
長さの担保は
[ADR-0045](../adr/0045-footer-file-externalization.md) F5 で決着済み:
warn 閾値は設けず、rebuild ログで各層の由来と文字数を常時可視化する。
本件に残る論点は文面 (案 A/B) のみ。
...
- [ADR-0045](../adr/0045-footer-file-externalization.md) (accepted、
  実装は phase 未採番) が実装されれば文面の差し替えに server 再ビルド
  が不要になり、「案 A から始めて不足分を計測する」進め方の試行
  コストが下がる
