...
| `timeout` | peer processing timed out (Antigravity: a tool step past the absolute `KAOIRO_ANTIGRAVITY_TOOL_TIMEOUT_MS` deadline ended the turn with `error_detail: tool_timeout`, [Antigravity tools and permissions](../reference/engines/antigravity-tools-permissions.md)) | Wait, then retry. |
