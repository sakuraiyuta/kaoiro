...
| ラッパー → サーバ | `directory_request` | `{}`。inter-agent messaging で wrapper が persona 名 → agent_id 解決を行うための peer 一覧取得。 サーバは `AgentStates.snapshot()` から **送信元 wrapper を除外** し、 各 entry を `{agent_id, persona: {id, name, sprite_set}, state, engine?, model?, effort?}` に丸めて `{:ok, %{agents: [...]}}` で reply(optional 3 field は non-empty string のときだけ載る、#102)。 wrapper 側は `mcp__kaoiro__list_agents` ツールでこれを呼ぶ。開示 field の正本は [protocol-inter-agent](protocol-inter-agent.md)「peer directory の情報境界」 |
