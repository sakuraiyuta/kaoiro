...
| サーバ → クライアント | `snapshot` | `{ agents: { <agent_id>: envelope }, snapshot_incomplete?: true }`。join 直後に push。`agents` は `TransportLimits` の frame bound に収まる wire projection で、entry が省略されたときだけ `snapshot_incomplete: true` を明示する。受信側は incomplete を全件同期済みと扱わず可視化する |
...
| サーバ → クライアント | `delivery_snapshot` | `{ deliveries: { <agent_id>: { issued_seq, acked_seq, pending_since? } } }`。join 直後に push。`deliveries` は issue #237 の recipient-local dispatch confirmation watermark で、未確認 gap の観測用であり再送 queue ではない。**operator 限定**: viewer join は常に `deliveries: {}` ([ADR-0048](../adr/0048-task-aggregation-delivery.md) addendum) |
...
差分追跡や再送要求は不要(agent_id ごと last-write-wins)。順序保証・重複排除の整列
