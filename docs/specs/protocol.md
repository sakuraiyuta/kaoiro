...
| `task`(予約) | **予約** | subagent/workflow の起動/更新/完了を通知する専用 type(スキーマ確定済み・実装は [subagent-tasks](subagent-tasks.md) 段階1 で未着手)。`{ kind: "started" \| "updated" \| "completed", agent_id, task_id, task_type, status, subagent_type?, workflow_name?, description?, usage?, last_tool_name?, summary?, skip_transcript? }`。`kind` がライフサイクルイベント種別、`status` は粗い状態(`running`/`completed`/`failed`/`stopped`)、`task_type` は拡張可能 enum(初期 `subagent`\|`workflow`。`tasklist` を追加決定済み — エージェント自身の todo リスト全体を optional `items` で運ぶ単一エンティティ・全体置換、[ADR-0049](../adr/0049-tasklist-on-task-envelope.md)。受信側は未知値を汎用表示へフォールバック)。親 `state_change` とは独立し、親 `agent_id` 参照で紐づく子エンティティを運ぶ。`kind=updated` は wrapper 発行側で間引き(`started`/`completed` は即時)。後続接続へは既存 snapshot 枠で active set を一括提供([ADR-0019](../adr/0019-subagent-workflow-entity-and-task-envelope.md) / [ADR-0047](../adr/0047-task-envelope-schema.md) / [ADR-0048](../adr/0048-task-aggregation-delivery.md))。予約追補のため `version` 据え置き |
