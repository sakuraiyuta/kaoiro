...
| サーバ → runner | `reset_session` | `{ version, agent_id, mode, request_id, previous_session_id? }`。session_reset relay。runner supervisor は `handleResetSession` で同 agent entry を kill + fresh relaunch(resume_session_id なし、resume_snapshot は保持で phase-15 D8 の最終 effective 値を再適用)、spawn 例外なら旧 `previous_session_id` で resume 再試行(rollback)、失敗で `rollback_failed` + entry drop(disconnected)([ADR-0036](../adr/0036-session-lifecycle-commands.md) F2) |
