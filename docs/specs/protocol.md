...
| 方向 | イベント | payload |
|---|---|---|
| runner → サーバ | `register` | `{ host_id, cwd_allowlist, allowed_personas? \| blocked_personas? \| personas?, capabilities?, engines?, build_revision?, build_dirty? }`。接続時に 1 回。選択可能 cwd 許可リスト(#22)と persona 受け入れ方針を申告。persona 方針は `allowed_personas`(id allowlist)/ `blocked_personas`(id blocklist)/ 3 者とも未設定(accept-all)の**排他 1 択**で、複数設定は fail-loud な invalid register([ADR-0031](../adr/0031-runner-persona-trust-mode.md))。`personas`(`WirePersona[]`)は legacy・deprecated で `id` のみ allowlist として解釈され、`name` / `sprite_set` は server SoT([ADR-0029](../adr/0029-persona-server-sot-and-pack-distribution.md))が持つため無視 + deprecation warn。`engines` は engine ごとの起動 catalog(`[{ id, models: [EngineModelInfo] }]`)で、LaunchDialog の engine → model → effort カスケードを wrapper プロセス不在のまま構成するために送る([ADR-0032](../adr/0032-codex-adapter.md) F4bc)。`capabilities` の値集合は `"claude-code" \| "codex"` ([ADR-0032](../adr/0032-codex-adapter.md) F4a)。旧値 `"claude"` は 1 リリース互換窓で `"claude-code"` にサイレント正規化して deprecation warn、次リリースで厳格 reject ([ADR-0032](../adr/0032-codex-adapter.md) F4a、2026-07-10 確定)。dashboard 側は 2 種以上のとき LaunchDialog に engine セレクトを出す。`build_revision`(full 40 桁 git SHA、未確定なら `"unknown"`)/ `build_dirty`(bool)は build identity(issue #228、[ADR-0053](../adr/0053-build-identity.md))— この `version` フィールドとは別軸で、両方とも省略可(pre-#228 runner との互換)、型崩れは値ごと無視して drop する(fail-closed、spoofing 対策) |
