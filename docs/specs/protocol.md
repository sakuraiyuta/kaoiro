...
targets}` で、競合する lifecycle 操作は `agent_busy` で拒否する。
...
`transition_id` が exact match したときだけ intent を閉じ、同じ
snapshot へ error なし `reconnected` inform を配る。不一致 token は
復帰と見なさない。intent の timeout / terminal failure は、
AgentStates がまだ `disconnected` のときだけ通常の
`error.code=disconnected` へ昇格する。`spawn_failed` は reset の
...
structured peer error guidance を解せる必要がある。
...
| サーバ → runner | `restart` | `{ agent_id, request_id? }`。**operator 限定**。新 server は live agent の予告済み再起動に non-empty `request_id` を採番する。新 runner はこれを relaunch 後 wrapper config の `transition_id` に置き換える。旧 server の省略を受けた新 runner は従来の restart 挙動を保つ(issue #266) |
