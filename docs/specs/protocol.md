...
| クライアント → サーバ | `list_users` | `{ version }`。**operator のみ**(issue #207)。`list_conversations`(issue #276。ドキュメント化されておらずこの表に行は無い)と同型の pure read-time query — dashboard の SettingsDrawer が開かれた時にオンデマンドで取得し、live push は無い。runner には一切中継されない。`Users.all_with_role/1` を synchronous に呼び、reply は `{ users: [{ id, kind, display_name, role }] }`。この allow-list (id/kind/display_name/role) は issue #207 が operator 向け開示として**独立に決定**したものであり、[ADR-0021](../adr/0021-role-information-disclosure-policy.md) F6-8 が定める agent 向け allow set (`directory_request` の `users` 投影) とは別実装 — 中身が一致しているのは F6-8 の流用ではなく偶然の一致(ADR-0021 F6-1)。role が auth SoT (OAuth allow-list / token config) から解決できない user は `Users.all_with_role/1` 側で結果から除外される。`Users` の内部専用フィールド `source` はワイヤに一切乗らない |
