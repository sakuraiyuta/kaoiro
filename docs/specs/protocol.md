...
| 外枠(フレームキー) | エンベロープ直下の固定キー集合 `version`/`agent_id`/`session_id?`/`persona`/`display_name?`/`ts`/`type`/`state`/`payload`/`ext`。v0 で固定済み(`session_id?` は optional、後述)。`display_name`(issue #219 D19)は `persona.name` と独立した mutable な通称 — 稼働中の rename が書き換えるのはこちらのみ、`persona`(canonical、session 中不変)は触らない。旧 wrapper build との互換のため optional |
...
| ラッパー → サーバ | `delivery_ack` | `{ delivery_seq: positive integer }`。issue #247 の SDK-dispatch confirmation watermark。capability 未交渉、duplicate、future は no-op。再送要求ではない。 |
...
rollback wrapper 起動成功を意味するため、issue #258 の failure 通知と
...
| サーバ → runner | `stop` | `{ agent_id }`。**operator 限定**。server は client payload の `host_id` が `AgentId.host_id_from(agent_id)` と一致することを intent mutation より前に検証する。active planned intent は stop を阻害せず、server が cancel + tracked peer への terminal `disconnected` を済ませてから relay する(issue #266) |
| サーバ → runner | `restart` | `{ agent_id, request_id? }`。**operator 限定**。server は client payload の `host_id` が `AgentId.host_id_from(agent_id)` と一致することを intent mutation より前に検証する。新 server は live agent の予告済み再起動に non-empty `request_id` を採番する。新 runner はこれを relaunch 後 wrapper config の `transition_id` に置き換える。旧 server の省略を受けた新 runner は従来の restart 挙動を保つ(issue #266) |
