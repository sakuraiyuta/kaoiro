...
| `result` | **確定** | `{ text?: string, is_error?: boolean, error_subtype?: string, error_detail?: string }`。ターン完了時の最終応答。`is_error` でエラー終了を区別し、`error_subtype` に SDK の終了 subtype(`max_turns` / `during_execution` / `max_budget` / `max_structured_output_retries` 等)、`error_detail` に SDK が返した本文(`SDKResultError.errors` を `"; "` 連結、無ければ `stop_reason`)を**整形せず**載せてクライアントへリレーする(#127)。どちらも成功時は absent。エラー本文を生のままリレーする方針は [ADR-0016](../adr/0016-error-body-relay.md) 由来だが、同 ADR の単一 `error_message` 案は実装されず、UI 分岐可能な subtype と本文の 2 フィールドに置き換わった(ADR-0016 が併記した「wrapper プロセス異常終了時に落ちる直前のエラーを送る」も未実装)。state は `done`/`error` の後 `waiting_input`。累計コスト USD は `ext.cost` に付与(#8)。`log` と同様 **operator 限定配信**([ADR-0012](../adr/0012-response-display-and-dashboard-scope.md)) |
...
| ラッパー → サーバ | `session_reset_request` | `{ mode: "new" \| "clear", reason?: string }`。[ADR-0043](../adr/0043-agent-initiated-session-reset.md) による agent 自身の deferred reset 要求。Claude wrapper の broker 承認済み `request_session_reset` tool が**当該 turn の result 処理後**に、自身の wrapper channel からのみ送る。server は agent_id を payload から受け取らず接続に bind し、capability / KaoiroState / dispatch-cooldown / pending lock を既存 `SessionResets.check_and_acquire` で atomic 検証して operator 経路と同じ runner flow へ合流させる。`reason` は string かつ既存 frame 上限内のみ受理し、runner payload へは渡さない。reject は既存 lifecycle 語彙 (`agent_busy` / `session_reset_pending` / `unsupported_session_reset` / `runner_unavailable` 等) を用い、新語彙を追加しない |
...
| サーバ → runner | `switch_session` | `{ agent_id, resume_session_id, request_id? }`。**operator 限定**。稼働中 agent の resume 先を差し替える(agent_id / cwd は不変)。runner は entry の resume ポインタを付替え、F4 ロックを旧→新へ移送してから wrapper を再起動(kill → relaunch)。T3(cwd 配下実在)と F4(同一 session を別 agent が resume 中でない)は runner が再検査。失敗は `spawn_result` で報告([ADR-0014](../adr/0014-session-resume-and-restore.md) resume-swap)。live switch は SDK session id を再利用するため、`request_id` が「この switch が生んだ接続」を旧接続と区別する唯一の手段になる(phase-27) |
...
| クライアント → サーバ | `refresh_engine_catalog` | `{ engine, request_id, force? }`。**operator 限定**。LaunchDialog の「モデル一覧を再取得」。サーバは検証後 `runner:<host_id>` へそのまま relay する(上記 runner 制御メッセージ、[ADR-0039](../adr/0039-engine-catalog-live-probe.md)) |
