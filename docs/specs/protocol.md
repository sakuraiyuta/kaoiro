...
| クライアント → サーバ | `rename_agent` | `{ version, agent_id, name }`。runner へは中継されないが ADR-0015 は「3 者すべてのメッセージ」を対象とし例外を設けないため、`version` は必須(issue #197 段階3 ふじ MF-1 レビュー指摘、`launch_defaults` と同じ扱い)。**operator のみ**(issue #197 段階3、#198 まで固定)。`name` は trim 後 non-empty / 64 grapheme cluster 以下 / 制御文字 (C0+DEL) 禁止 — spawn 時の `name?` (上記) と同じ規則。live / disconnected どちらの agent でも受理。`AgentDirectory.rename/2` が唯一の authoritative write (revision を単調 +1)、成功で `{ persona, revision }` を reply しつつ (a) `wrapper:<id>` topic へ `persona_sync` を relay (下記、fire-and-forget) と (b) 既 join operator 全員の `directory` push を live 更新 (`entries` フルスナップショット、viewer には届かない)。未知 agent は `unknown_agent`、name 違反は `invalid_name`、対象 agent の revision が safe-integer 上限 (下記) に既に達している場合は `revision_exhausted`(fail-closed、書き込み・relay とも一切行わない、issue #197 段階3 ふじ MF-5 レビュー指摘) |
| クライアント → サーバ | `rename_user` | `{ version, user_id, name }`。`version` は `rename_agent` と同じ理由で必須(issue #197 段階3 ふじ MF-1 レビュー指摘)。**operator のみ、任意の既存 user が対象**(issue #197 段階3)。`name` の contract は `rename_agent` と同じ。`Users.rename/2` を synchronous に呼び、成功で更新後の `{ id, kind, display_name }` を reply。`directory_request` の `users` 投影 (issue #197 段階2) は毎回 live 読みのため、これ以外の broadcast は無い。未知 user は `unknown_user`、name 違反は `invalid_name`、user_id の charset 違反 (issue #61) は `invalid_user_id` |
...
| サーバ → ラッパー | `persona_sync` | `{ version, name, revision }`(issue #197 段階3)。`version` は live relay・after-join push 両 producer が常に付与する(issue #197 段階3 ふじ MF-1 レビュー指摘)。ラッパー側は完全一致のみ正常とみなし、欠落/不一致は warn した上で name/revision が valid ならベストエフォートで受理継続する(ADR-0015)。**join のたびに無条件で push**(fresh join・reconnect 問わず、`AgentDirectory` の現在値を after_join handshake から。subscribe 確立後に送るので join/3 内での早期 read には無い race が閉じている) に加え、`rename_agent` 成功時に live relay(fire-and-forget、`wrapper:<agent_id>` topic broadcast、offline wrapper には届かず after-join sync で追いつく)。`revision` は agent_id ごとの単調カウンタ (`AgentDirectory.rename/2` が発行) — ラッパーは自分が最後に適用した revision 以下の push を無視する(2 つの `rename_agent` が broadcast 順を入れ替えて届いても、古い方には巻き戻らない)。適用すると `persona.name` のみ書き換え、`persona.id` / `sprite_set` と注入済み personality prompt は不変のまま、直ちに `state_change` を再送する(次の自然発生イベントを待たない — idle/waiting_input は無期限に待たされ得るため)。ack の保証範囲は `AgentDirectory` への commit までで、ラッパー側の反映は eventual convergence(relay loss は rollback せず、次の実 reconnect 時の sync で回復する)。**wire 上の domain**(issue #197 段階3 ふじ MF-4/MF-5 レビュー指摘): `name` は `rename_agent`/`rename_user` と同じ trim/64 grapheme cluster/制御文字禁止の contract、`revision` は **0 以上 `Number.MAX_SAFE_INTEGER` (2^53-1) 以下の safe integer** — `AgentDirectory.rename/2` が実際に発行し得る値の範囲そのもの(上限に達した agent への rename は `revision_exhausted` で fail-closed に拒否され、この範囲を超える revision は発行されない)。ラッパー側 narrow (`transport.ts` の `persona_sync` ハンドラ)はこの両方を value-level で検証し、違反する push は丸ごと drop する。poisoning のリスクがあるのは safe-integer 上限を超える巨大な正数の側であって負数側ではない — server は authoritative な revision を発行しているのに、その値を wrapper 側が `Number.isSafeInteger` で drop し続けるため、reconnect を挟んでも当該 agent の表示名が永久に収束しなくなる。負の revision を drop するのは guard poisoning 対策ではなく、producer (`AgentDirectory.rename/2`、常に 0 以上しか発行しない) の domain 違反を検出しているだけであり、`host.renamePersona` の `revision <= 現在値` guard は baseline `0` から出発するため負値がそもそも植わることはない |
