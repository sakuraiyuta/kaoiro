...
- **CLI only (GUI disabled)**Note CUI and headless hosts.
...
(`pnpm deploy --legacy`)
- For actual demand**2 arch(`darwin-arm64` / `linux-x64`)**Note 4
Don’t make it
- Cross-build only injects `supportedArchitectures` of pnpm
(check that you can generate both from one darwin host)
- The installation is "decompression → Edit configuration file → One command execution". permission
...
tarball**rate****After installation**Note
...
**source origin and activation layout**Note confusing
...
- Therefore, the form that was called "repo-direct" is
  **local-build release profile**permission repo**build**
...
- **After the switch is stopped,  ic is performed with linklink + `rename(2)`.**Note `mv`
unusable — `mv` follows it
GNU coreutils 9.4
link remains in the old release. GNU `mv -T` is correct
call `rename(2)` via node
- **Keep the previous release as `previous`**Note Default of retention generations 3
...
- **The startup sim does not build only verify**Note The test object is generated by builder
...
refusal** (issue #219 Note review). manifest cannot be your witness
Note **Re-derive input is `package.json` in the same tree, so this is
...
Not available
- **`PartOf` / `BindsTo`**Stop   isthe relevant entryagated in another route
...
HOME**`dirty` says, "This SHA does not have a medium."**Note In other words
...
|||
|---|---|
| **activation** (`current`id)| **Clean 40 digits   only**.`-dirty` / `unknown`Note`--allow-dirty`to the dev host|
| **Clean release** | **No replacement**Note reinstall is no-op because content-addressed. No flags to replace|
...
`releases/<clean-id>/`
No change as a custom**Irregular**permission If you suspect the damage, you can delete it by hand —
...
**Cannot verify multiple lines**Note Survey (2026 16): VERSION
...
|install / switch script and Splink +`rename(2)`atomicity| **OS Common Contract**Note Linux**macOS** |
...
|||
|---|---|
| experimental API | `vm.SourceTextModule`Note`--experimental-vm-modules`without flags`undefined`(in node v24.3.0). Node can change without notice|
|Version|permission`--disable-warning`is added in Node >= 20.11 / 21.3.`engines.node >= 22`always exists, but older nodes fall at the time of flag interpretation|
...
#### Declaration of execution reference — notthe relevant entry(2026 16)
...
|Paths assembled during execution|Package`package.json`Note`kaoiro.runtimeAssets` |
...
**This heuristic itself is taking it once**Note `new URL(` /
`import(` / `require(`
