...
将来増える見込みの field (例: `supports_model_switch`, `supports_effort_switch`, `supports_cwd_tracking`) は本 ADR の枠内で追加可能。追加時は envelope schema 追補 + agent-common の型追加のみで済む。
