...
protocol [0015-protocol-version-stamping](0015-protocol-version-stamping.md)
wire protocol compatibility, not the identity of thefactfact. Note
...
**`built_at` is a diagnostic-only field on the the relevant entry side and on the the relevant entry side
Issue #218 round 2 advisory 2, Note
`BuildInfo`(TS type,
...
The implementation can be erroneous as "built at is required for the relevant entry". `built_at`
...
**`built_at` is also subject to value verification (issue #218 round 3, Note revert MF-4).**
...
**Round 2 Judgment (Note Reversing and MF-2):** `git status --porcelain`
...
This calculation is**repo-level `scripts/build-identity.mjs` only one place**Note
...
**round 3 arbitration (Note revert, MF-3): `--format` also validates the same arbitration
...
Note**degrade to the same `unknown` as  **that — to the same file
...
pattern, but this was judged to be incorrect in round 2 (Note revert,
...
**(issue #218 round 3 advisory 2, Note):**
...
is unrelated).**`"unknown"`**and this athe relevant entrymetry
Thanks**`"unknown"`**can be determined.

### Validation of value range (round 2, Note MF-3)

`revision` is a 40-digit the relevant entry
...
**extended state transition in round 2 (Note revert, MF-4).**round 1
...
**Round 3 found a dirty side hole (Note revert and MF-1).** round 2
...
**round 3 dashboard side pair invariant also found (Note)
...
Fail-open dashboard Note trust boundary
...
config check**Note**to entry point.
...
Not solved.
- round 2 (Note revert) returns the following: server identity from ENV
