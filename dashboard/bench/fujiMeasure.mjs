...
try{
 ...
 for(const cfg of matrix){
  const {run,mode,tick,history}=cfg;const label=`${prod?"prod":"dev"}-${cfg.source??"both"}-${mode}-tick${tick}-h${history}-r${run}`;
  ...
  await page.waitForTimeout(1500);await page.locator('button.open',{hasText:'agent-viewed'}).click();await page.waitForSelector('.log');await page.waitForTimeout(700);await page.locator('textarea').first().focus();
  ...
  const raw=await page.evaluate(()=>{window.fuji.active=false;return{...window.fuji,timer:undefined,timeOrigin:performance.timeOrigin,text:document.querySelector('textarea').value,dom:document.querySelectorAll('*').length,rows:document.querySelectorAll('.log-entry').length}});
