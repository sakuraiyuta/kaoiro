...
describe("server -> dashboard event bindings carry version checks (issue #270)", () => {
  ...
  it("T3-1: policy の19種すべてで欠落をwarnし、受信を継続する", async () => {
  ...
  it("T3-2: policy の19種すべてで一致versionは無警告", async () => {
    const { ws } = await connectAndJoin();
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
  ...
  it("T3-3: policy の19種すべてで不一致versionをwarnし、受信を継続する", async () => {
    const { ws } = await connectAndJoin();
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
  ...
  it("T3-4: policy の19種すべてを一回ずつだけ bind する", async () => {
