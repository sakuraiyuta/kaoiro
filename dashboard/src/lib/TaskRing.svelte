...
<!-- 頭上リング (issue #180, ADR-0019/0047/0048): サブエージェント/workflow
     ...
     aria-hidden は付けず role="img" + aria-label で読み上げ対象にする)。
     呼び出し側は {#key state}...{/key} の外に置くこと — state 遷移
     (dissolve remount) の影響を受けず単独で回り続ける。数値は出さない
     (on/off のみ)。AgentCard / AgentDetail 共有(issue #180 follow-up,
     2026-08-10 — マスター指摘: 頭上リングが AgentDetail に無いのはマスター
     未承認のスコープ外判断だったため追加)。 -->
<span
  class="task-ring"
