defmodule KaoiroServer.PermissionModes do
  ...
  @doc """
  Removes the agent's persisted mode from memory + DETS. Idempotent — an
  unknown agent returns `:ok`. Synchronous so the operator-driven delete
  ...
  def handle_call({:delete, agent_id}, _from, state) do
    ...
    {:reply, :ok, %{state | modes: Map.delete(state.modes, agent_id)}}
  end
